...
typedef enum {
    ...
    SB_PLUS, SB_MINUS, SB_TIMES, SB_SLASH,  // Thêm vào dây
    SB_LPAR, SB_RPAR, SB_LSEL, SB_RSEL
} TokenType;

typedef struct {
    char string[MAX_IDENT_LEN + 1];  // Kích thu?c m?ng string h?p lý
    int lineNo, colNo;
    TokenType tokenType;
    int value;
} Token;

// Ki?m tra t? khóa và tr? v? ki?u TokenType tuong ?ng
TokenType checkKeyword(char *string);

// T?o token m?i t? ki?u TokenType, s? dòng và c?t
